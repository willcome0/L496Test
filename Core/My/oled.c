...
const uint8_t DCB2HEX_TAB[8] = {0x80, 0x40, 0x20, 0x10, 0x08, 0x04, 0x02, 0x01};
const char FONT14x14TAB[50] =  "康任务：优先级剩余栈状态序号";
const uint8_t FONT14x14[50][24] =
{
...
void OLED_Show_ZH12_12(uint8_t x, uint8_t y, uint8_t ch1, uint8_t ch2, uint8_t mode)
{
    ...
    {
        //需同时判断汉字的低字节和高字节
        if (FONT14x14TAB[i]==ch1 && FONT14x14TAB[i+1]==ch2)
            break;
    }
    ch1 = i/2;
...
//      for(j=0; j<2; j++) //每行2字节
//      {
//         font_dat = FONT14x14[ch1][i*2+1-j];
//         for(k=0; k<8; k++)
